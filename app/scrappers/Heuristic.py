class Heuristic:
    ...
    def normalize(self):
        ...
        ratesScore = [weights[round(product.rate-1)] *
                      product.reviewsCount for product in self.products]
        ...
        for product in self.products:
            ...
            if max_min_price_diff != 0:
                normalizedTotalPrice = (
                    product.price - minTotalPrice)/(max_min_price_diff)
            if max_min_rate_diff != 0:
                normalizedRate = (weights[round(
                    product.rate-1)] * product.reviewsCount - minRateScore)/(max_min_rate_diff)
